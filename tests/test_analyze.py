...
class TestHighestPriorityPath:
    ...
    def test_chain_is_deterministic_under_ties(self, mgr):
        """Two equal-priority parallel chains must produce the same chain
        every call, and ties resolve to the alphabetically-first endpoint."""
        from callback_helpers import _compute_highest_priority_path

        _setup_graph(mgr, [
            _make_node("A", value=5, interest=5, difficulty=1),
            _make_node("B", value=5, interest=5, difficulty=1),
            _make_node("X", value=5, interest=5, difficulty=1),
            _make_node("Y", value=5, interest=5, difficulty=1),
        ], edges=[
            ("A", "B", EDGE_NEEDS_HARD),
            ("X", "Y", EDGE_NEEDS_HARD),
        ])

        runs = [_compute_highest_priority_path(mgr) for _ in range(20)]
        assert all(r == runs[0] for r in runs), f"Non-deterministic: {set(map(tuple, runs))}"
        assert runs[0] == ["A", "B"], f"Expected alphabetic tie-break to ['A','B'], got {runs[0]!r}"

    def test_chain_prefers_shorter_on_dp_tie(self, mgr):
        """When two chains have equal cumulative dp but different lengths,
        the shorter chain wins. Mirrors the production-graph Goal-tail tie
        where extending into 0-score Goal containers doesn't change dp but
        does pad the chain."""
        from callback_helpers import _compute_highest_priority_path

        # Long chain: A (val 5) -> G1 -> G2 -> G3 (Goals score 0). dp=5, length=4.
        # Short chain: B (val 5) -> G4 (Goal). dp=5, length=2.
        # Length tiebreak should pick the short chain. Note B > A alphabetically,
        # so name tiebreak alone would have picked the long chain — this test
        # verifies length wins over name.
        _setup_graph(mgr, [
            _make_node("A", value=5, interest=5, difficulty=1),
            _make_node("B", value=5, interest=5, difficulty=1),
            _make_node("G1", type="Goal"),
            _make_node("G2", type="Goal"),
            _make_node("G3", type="Goal"),
            _make_node("G4", type="Goal"),
        ], edges=[
            ("A", "G1", EDGE_NEEDS_HARD),
            ("G1", "G2", EDGE_NEEDS_HARD),
            ("G2", "G3", EDGE_NEEDS_HARD),
            ("B", "G4", EDGE_NEEDS_HARD),
        ])

        chain = _compute_highest_priority_path(mgr)
        assert chain == ["B", "G4"], f"Expected shortest tied chain ['B','G4'], got {chain!r}"

    def test_chain_prefers_shorter_at_intermediate_step(self, mgr):
        """Length tiebreak applies at every DP step, not just the endpoint.
        If two predecessors of node Z have equal dp but different chain
        lengths, the shorter-prefix predecessor must be chosen."""
        from callback_helpers import _compute_highest_priority_path

        # Two paths to Z with equal dp but different lengths:
        #   X (val 5) -> Z              : dp[X] = 5, dp[Z] = 5+score(Z), length=2
        #   Y1 (Goal 0) -> Y2 (Goal 0) -> Y3 (val 5) -> Z
        #     : dp[Y3] = 5 (Goals contribute 0), dp[Z] = 5+score(Z), length=4
        # Both paths produce equal dp[Z]; X is the shorter-prefix parent.
        # Then Z -> END (val 1) extends to a deterministic endpoint.
        _setup_graph(mgr, [
            _make_node("X", value=5, interest=5, difficulty=1),
            _make_node("Y1", type="Goal"),
            _make_node("Y2", type="Goal"),
            _make_node("Y3", value=5, interest=5, difficulty=1),
            _make_node("Z", value=5, interest=5, difficulty=1),
            _make_node("END", value=1, interest=1, difficulty=1),
        ], edges=[
            ("X", "Z", EDGE_NEEDS_HARD),
            ("Y1", "Y2", EDGE_NEEDS_HARD),
            ("Y2", "Y3", EDGE_NEEDS_HARD),
            ("Y3", "Z", EDGE_NEEDS_HARD),
            ("Z", "END", EDGE_NEEDS_HARD),
        ])

        chain = _compute_highest_priority_path(mgr)
        # Chain should walk through X, not Y1->Y2->Y3
        assert "X" in chain, f"Expected shorter-prefix predecessor 'X' in chain, got {chain!r}"
        assert "Y1" not in chain and "Y2" not in chain, (
            f"Expected long Goal-prefixed branch to be skipped, got {chain!r}")
