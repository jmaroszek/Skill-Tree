...
def test_core_engine_noop_on_non_graph_tab_switch(monkeypatch, tab):
    ...
    # core_engine accepts 47 positional args; supply Nones and let the guard fire
    # before any of them are used.
    args = [None] * 75
    # active_tab sits at positional index 41 (see core_engine signature; total 75 args)
    args[41] = tab
    result = cb(*args)
...
def test_core_engine_runs_when_trigger_is_not_main_tabs(monkeypatch):
    ...
    args = [None] * 75
    args[41] = "tab-settings"  # even though tab is non-graph, trigger is not main-tabs
    try:
        result = cb(*args)
    except Exception:
