...
The window opens with a contributors chart. It ranks the pieces that drive the recommendation, so you can see what is pulling this node up the list. Each bar is that piece's share of the node's total value. The node's own ratings are one bar. The rest are the nodes whose value flows back to it. The bars are colored by where the value comes from: the node itself, a hard prerequisite, a soft prerequisite, or a synergy link. Hover over a bar to see how far away that node is and how much of its value reaches this one.
